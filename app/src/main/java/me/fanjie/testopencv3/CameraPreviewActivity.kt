...
class CameraPreviewActivity : AppCompatActivity() {
    ...
    companion object {
        fun start(context: Context) {
            context.startActivity(Intent(context, CameraPreviewActivity::class.java))
        }
    }
    ...
    private fun initEmitter() {
        val flowable1 = Flowable.create<Frame>({ emitter1 = it }, BackpressureStrategy.LATEST)
        val flowable2 = Flowable.create<Frame>({ emitter2 = it }, BackpressureStrategy.LATEST)
        Flowable.zip(flowable1, flowable2, BiFunction<Frame, Frame, Frame> { frame1, frame2 ->
